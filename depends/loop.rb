def socketloop()
  ...
  if $reloaded == 1
	...
	if $host == "PING"
	  ...
	  case $second
	    when "PRIVMSG"
		  puts "(#{$splits[1]}) <#{$host[0,$host.index("!")]}> #{$message}"
		  
			if $splits[1] != $botname
				logtext("<#{$host[0,$host.index("!")]}> #{$message}", $splits[1])
			else
				logtext("<#{$host[0,$host.index("!")]}> #{$message}", $host[0,$host.index("!")])  
			end
			
		  if $message[1,$message.length - 2] == "VERSION"
            send_raw("NOTICE " + $host[0, $host.index("!")] + " :" + 1.chr + "VERSION Ruby-irc bot Version 4.2 by Umby24" + 1.chr) #CTCP Version.
		  ...
		  if $message[0, 5] == 1.chr + "PING" # Handles CTCP Pings.
            ...
            send_raw("NOTICE " + $host[0, $host.index("!")] + " :" + 1.chr + "PING " + thisid + 1.chr)
			else
			send_raw("NOTICE " + $host[0, $host.index("!")] + " :" + 1.chr + "PING" + 1.chr)
			end
            puts "Received CTCP PING"
          end
		  
		  #load 'plugins/message.rb' # For user defined message handling (Keeps this file uncluttered)
		  eventMessage()
		  ...
		  if $message[0,1] == $prefix
			...
			if $access != nil
            $access.each {|item|
              if item.strip.downcase == $host[0,$host.index("!")].strip.downcase
                $okay = true
              end
            }
			end
		...
		when "NOTICE"
		  ...
		  next
		when "TOPIC"
		  $topic[$splits[1]] = $splits[2][1, $splits[2].length - 1]
		  puts "Topic Updated to #{$splits[2][1, $splits[2].length - 1]}"
		  next
		  
		when "QUIT"
	      puts "**** #{$host[0,$host.index("!")]} left #{$splits[1]} (#{$splits[2]})"
          $authed.delete($host[0,$host.index("!")])
		  next
		...
		when "NICK"
		  puts $host[0,$host.index("!")] + "has changed their nick to " + $message
		...
		when "JOIN"
		  puts "** #{$host[0,$host.index("!")]} joined #{$splits[1]}."
